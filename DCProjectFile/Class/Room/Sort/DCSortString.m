...
+ (NSMutableDictionary *)sortAndGroupForArray:(NSArray *)ary PropertyName:(NSString *)name {
    ...
    if (ary.count <= 0) {
        [MBProgressHUD showTips:@"数据源不能为空"];
        return sortDic;
    }
    ...
    if ([objc isKindOfClass:[NSString class]]) {
        ...
        for (NSString *str in ary){
            DCSortString *sortString = [DCSortString new];
        ...
        for (NSObject *obj in ary){
            ...
            for (i = 0; i < propertyCount; i++) {
                ...
                if (i == propertyCount -1) {
                    [MBProgressHUD showTips:[NSString stringWithFormat:@"数据源中的Model没有你指定的属性:%@",name]];
                    return sortDic;
                }
            }
        }
    ...
    NSString *itemString;
    for (DCSortString *sort in sortAry){
...
+ (NSMutableArray *)sortAsInitialWithArray:(NSArray *)ary {
    ...
    for (NSInteger index = 0; index < ary.count; index++) {
        DCSortString *sortString = ary[index];
        sortString.englishString = [DCSortString transform:sortString.string];
